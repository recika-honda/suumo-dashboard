/**
 * Image Pipeline — OpenAI Vision 個別分類 + Sharp リサイズ
 *
 * 各画像を1枚ずつ Vision で分類し、正確なカテゴリを割り当てる。
 * 使用済みカテゴリは候補から除外し、重複を防ぐ。
 *
 * Model / detail は env で上書き可:
 *   OPENAI_VISION_MODEL  (default: "gpt-4o")
 *   OPENAI_VISION_DETAIL (default: "high")
 */
...
const VISION_DETAIL = process.env.OPENAI_VISION_DETAIL || "high";
    ...
    messages: [
      {
        ...
        content: [
          {
            type: "image_url",
            image_url: { url: `data:image/jpeg;base64,${b64}`, detail: VISION_DETAIL },
          },
          {
            type: "text",
...
/**
 * 画像を個別に OpenAI Vision で分類し、Sharp でリサイズ
 *
